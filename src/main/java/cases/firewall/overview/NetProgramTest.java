...
public class NetProgramTest extends SeleniumTestCase {
	...
	public void open(){
		...
		overViewPage.singleClick(overViewPage.getNetProgramAll(),overViewPage.getNetOpen());
		Assert.assertEquals(overViewPage.getWebDataTr().findElement(By.xpath("./td[2]"))
				.getText().trim(),expect.getString("open"),"已开启验证失败");
		Assert.assertTrue(overViewPage.getWebDataTr().findElement(By.xpath("./td[6]"))
	...
	public void close(){
		...
		overViewPage.singleClick(overViewPage.getNetProgramAll(),overViewPage.getNetClose(),4);
		...
		Assert.assertTrue(TestUtil.getValueList(overViewPage.getClientNames())
				...
				"已关闭验证失败");
		Assert.assertTrue(overViewPage.checkItems(overViewPage.getWebNets(),
