...
public class ActionTest extends SeleniumTestCase {
	...
	public void install(){
		...
		logPage.singleClick(logPage.getActionAll(),logPage.getInstall(),5);

		Assert.assertTrue(TestUtil.getValueList(logPage.getClientNames())
						.containsAll(TestUtil.JsonToList(expect.getJSONArray("installName"),"name")),
				"安装动作验证失败");
		Assert.assertTrue(logPage.checkItems(logPage.getWebActions(),
				expect.getString("installState")),"安装动作验证失败");
	...
	public void restart(){
		...
		Assert.assertEquals(logPage.getWebDataTr().findElement(By.xpath("./td[1]"))
				.getText().trim(),expect.getString("restartName"),"重启动作日志验证失败");
		Assert.assertEquals(logPage.getWebDataTr().findElement(By.xpath("./td[4]"))
