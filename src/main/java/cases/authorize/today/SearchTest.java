...
public class SearchTest extends SeleniumTestCase {
	...
	public void searchIp(){
		...
		Assert.assertEquals(todayPage.getWebDataTr().findElement(By.xpath("./td[2]/div")).getText().trim(),
				ip,"ip地址筛选验证错误");
	}
	...
	public void searchMac(){
		...
		Assert.assertEquals(todayPage.getWebDataTr().findElement(By.xpath("./td[3]/div")).getText().trim(),
				mac,"mac地址筛选验证错误");
	}
}
