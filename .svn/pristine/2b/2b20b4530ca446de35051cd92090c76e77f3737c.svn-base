...
public class RoutinePage extends BasePage {
	...
	public void buttonTwoUtil(EventFiringWebDriver driver){
		...
		((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView();", worm);
		click(worm);
		click(rain);
		click(witkey);
		click(dll);
	...
	public void inputUtil(EventFiringWebDriver driver,String count,String sName,String addr,String sPort){

		TestUtil.seleniumWait();

		((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView();", queryCount);
		queryCount.clear();
