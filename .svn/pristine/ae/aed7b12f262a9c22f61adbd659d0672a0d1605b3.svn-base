...
public class BasePage {
	...
	//时间范围验证通用工具方法
	public void timeFrameForOne(WebElement element) {

		click(timeNolimt);
		TestUtil.seleniumWait();
		click(element);
		TestUtil.waitForElementNumber(By.xpath("//table[@id='tbClient']/tbody/tr/td[1]"),1);
	...
	public void click(WebElement element) {
		...
		if(bundle.getString("delayclick").equalsIgnoreCase("true")){
			TestUtil.seleniumWait(5000);
		}

		new WebDriverWait(driver, 10).until(ExpectedConditions.elementToBeClickable(element)).click();
	}

	public void changeBlock(WebElement element){

		TestUtil.setAttribute(element,"style","display: block;");
		TestUtil.seleniumWait();
		click(element);
	}

	public void addAttribute(WebElement element){

		TestUtil.addAttribute(element,"style","display: block;");
		TestUtil.seleniumWait();
		click(element);
	}
